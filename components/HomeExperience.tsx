...
function HomeBody({active,setActive,s,trust,owner,business}:{active:number;setActive:(i:number)=>void;s:(typeof services)[number];trust?:ReactNode;owner?:ReactNode;business?:ReactNode}){return <main className={'site tone-'+s.tone}>
 <section className="heroScene">
  <Image className="heroVideo" src="/media/IMG_3027.jpg" alt="Freshly pressure cleaned concrete driveway and carport at a Cairns house" fill priority sizes="100vw" quality={60}/><HeroVideo/>
...
/* The 3.9 MB hero video only loads on bigger screens with motion allowed and
   no data saver. Phones get the still photo (next/image, sized for them),
   which is also what paints first everywhere. */
function HeroVideo(){const [src,setSrc]=useState<string|null>(null);useEffect(()=>{const c=(navigator as Navigator&{connection?:{saveData?:boolean}}).connection;if(c?.saveData||!window.matchMedia('(prefers-reduced-motion: no-preference)').matches)return;const phone=window.matchMedia('(max-width: 850px)').matches;/* Phones start the video after the page has loaded so the photo still paints first. They try the 0.6 MB portrait cut, and fall back to the full video until that file is on the site. */const go=()=>setSrc(phone?'/media/pressure-hero-mobile.mp4':'/media/pressure-hero.mp4');if(!phone||document.readyState==='complete'){go();return}window.addEventListener('load',go,{once:true});return()=>window.removeEventListener('load',go)},[]);if(!src)return null;return <video className="heroVideo" autoPlay muted loop playsInline preload="auto"><source src={src} type="video/mp4"/>{src!=='/media/pressure-hero.mp4'&&<source src="/media/pressure-hero.mp4" type="video/mp4"/>}</video>}
