...
function HeroVideo(){const [src,setSrc]=useState<string|null>(null);useEffect(()=>{const c=(navigator as Navigator&{connection?:{saveData?:boolean}}).connection;if(window.matchMedia('(min-width: 851px) and (prefers-reduced-motion: no-preference)').matches&&!c?.saveData)setSrc('/media/pressure-hero.mp4')},[]);if(!src)return null;return <video className="heroVideo" autoPlay muted loop playsInline preload="auto"><source src={src} type="video/mp4"/></video>}
